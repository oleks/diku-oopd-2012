...
the computer rather feebleminded, and hence it is the programmer that has to be
the clever one.

An explanation of the imperative paradigm seems hence to be pointless without a
gentle introduction to the inner workings of a computer, at least in some
abstract sense.

\begin{definition}

A \key{computer} is an entity with \key{state} and a series of
\key{processors}.  Processors perform \key{actions} that change the state of
the computer.

\end{definition}

While it may be modern (about time) to speak of computers as having multiple
processors, we will restrain ourselves to uniprocessor computers, unless
discussing multiprocessor computers is particularly interesting for some
particular concept, in which they will be mentioned explicitly.
...
in a finite period of time, call it $T_1$. This allows us to speak of a point
in time $T_n$ and $T_{n+1}$, as the time of action inception and termination,
respectively. Secondly, an action establishes a well-defined, intended
net-effect. This emphasises that the real objective of the computer's actions
is to perform deterministic computations that change the state of the computer.
