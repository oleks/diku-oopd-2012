...
output values, such that the mapping provides a \key{class of computations}
...
architectures deriving from John von Neumann's original
proposal\cite{von-neumann}. By this design, computers are rather feebleminded.
Hence, the programmer often has to be very clever about specifying the flow of
the program.
...
non-essential details, instead of concentrating on the problem at hand. The
paradigm, being ``so close to metal'', opens up for time spenders on various
frontiers. On the more boring side, the programmer may have to write a great
deal of code to do simple things, while on the more fun side, the programmer
can spend a great deal of time figuring out clever tricks to make the code
marginally faster on that particular architecture. Neither is a particularly
clever practise, and measuring programmer productivity by say the number of
lines written seems to be just picking the lesser of the two evils --
encouraging programmers to be sloppy (but ``productive'').

The more accurate productivity measure seems to be an evaluation of the class
of computations evocable by the written program, as well as the \key{time} and
\key{resource} \key{requirements} of such computations. Hence, the programmer's
job becomes the more poetic -- ``designing a class of computations'', rather
than the more mundane -- ''writing a program''. In essence, it seems more
fruitful for us to be more concerned with the \key{net-effect} of programs,
while the means of getting the program written are seemingly non-essential. 

This view may be acceptable so long as we don't consider that software has to
evolve. Either due to bugs or changes in the requirements. What's more,
software is usually evolved by people other than the original programmers. We
will come back to this thought after we've covered some basics of computer
architecture.
